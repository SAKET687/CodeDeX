...
export const fetchPlatformData = createAsyncThunk(
  ...
  async () => {
    ...
    //   const leetcodeUsername = localStorageData.leetcode.username;
    //   const leetcodeUrl = `https://leetscan.vercel.app/${leetcodeUsername}`;

    //   fetches.push(
    //     axios
    //       .get(leetcodeUrl)
    //       .then((res) => {
    //         console.log("LeetCode Response:", res); 
    //         console.log("LeetCode Response data:", res.data); 
    //         console.log("LeetCode Response data contest ranking:", res.data.contestRanking); 
    //         console.log("LeetCode Response data rating:", res.data.contestRanking.rating); 
    //         // Debugging: Print the entire response
    //         // Safely access the rating field, with fallback in case the structure differs
    //         const rating = res.data.contestRanking.rating;
    //         return { key: "leetcode", data: rating };
    //       })
    //       .catch((error) => {
    //         console.error("Error fetching LeetCode data:", error.response?.data || error.message);
    //       })
    //       .finally(() => {
    //         console.log("LeetCode fetch process completed");
    //       })
    //   );
    // }
    ...
    //       .get(leetcodeUrl)
    //       .then((res) => {
    //         return { key: "leetcode", data: res.data.contestRanking.rating };
    //       })
    //       .catch((error) => {
    ...
    if (localStorageData.leetcode.username !== "N/A") {
      fetches.push(
        axios
          .get(`https://leetscan.vercel.app/${localStorageData.leetcode.username}`)
          .then((res) =>
            ({ key: "leetcode", data: res.data.contestRanking })
          ));
    }
    ...
    responses.forEach((response) => {
      if (response.status === "fulfilled") {
        ...
        console.warn(
          `API fetch failed for ${response.reason?.config?.url || "unknown"
          }`
        );
      }
    });
